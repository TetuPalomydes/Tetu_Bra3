...
$.extend(Number.prototype, {
  ...
  toFormatTime: function (format) {
    ...
    if (h >= 100) format = format.replace("hh", h)
    else format = format.replace("hh", ("00" + h).substr(-2))
...
$.extend(TargetBusho.prototype, {
  ...
  analyze: function (elm) {
    var [dmyhp, hprem, hpmax] = $(elm)
        .find('DT:contains("HP")+DD')
        .text()
        .match(/([\d,]+)\/([\d,]+)/),
      [dmyjn, join] = $(elm)
        .find('DT:contains("参戦人数")+DD')
        .text()
        .match(/(\d+)\/\d+/),
      [dmyat, atk] = $(elm)
        .find('DT:contains("戦闘回数")+DD')
        .text()
        .match(/(\d+)\/\d+/),
      [dmbonus, bonus] = $(elm)
        .find('DT:contains("同盟ボーナス")+DD')
        .text()
        .match(/(\d+)/),
...
;(function ($) {
  ...
  $.extend(Log.prototype, {
    ...
    init: function (bufferCount) {
      if (bufferCount === undefined) this.max = 0
  ...
  var Display = (function () {
    ...
    function Dialog(options) {
      var $overlay = $(
          '<div id="imi_overlay"><div class="imc_overlay" /><div id="imi_dialog_container" /></div>'
        ),
      ...
      if (options.title) {
        $container.append(
          '<div class="imc_dialog_header">' + options.title + "</div>"
        )
      }
  ...
  $.extend(Page, {
    ...
    ajax: function (url, options) {
      return $.ajax(url, options).pipe(function (html) {
        var $html = $(html)
      ...
      $(document)
        .on("ajaxStart", function () {
          if ($("#imi_ajax_load").length == 0) {
            $("body").append(
              '<span id="imi_ajax_load" class="imc_ajax_load" style="display: none;"><img src="' +
                Data.images.ajax_load +
                '"></span>'
            )
          }
          $("#imi_ajax_load").show()
  ...
  $.extend(BushoCard.prototype, {
    ...
    analyze: function (elm) {
      try {
        ...
        this.pow_def =
          ((this.def_sw + this.def_sp + this.def_bw + this.def_hs) / 4) * 1.1 +
          2000
    ...
    click: function (eid, rem, type, stamina, deck) {
      ...
      var upURL =
        "/card/event_battle_attack.php?entry_id=" +
        eid +
        "&target_card=" +
        this.tid
      var false_flg = 0
      ...
      $.get("/card/event_battle_attack.php?" + paramstr, function (data) {
        ...
        .then(function () {
          return $.get(
            "/event_battle/npc_battle_check_general_enabled.php?entry_id=" +
              eid +
              "&SSID="
          )
        })
          ...
          return $.ajax({
            ...
            function (data, status, xhr) {
              ...
              if (result === undefined || result.length == 0)
                result = $("#resultBox-min", $body).text().trim()
              ...
              if (result !== undefined && result.length) {
                ...
                if (resultData.Skills == null) resultData.Skills = ["なし"]
                  ...
                  for (i = 0; i < resultData.Skills.length; i++)
                    [dummy, resultData.Skills[i]] = resultData.Skills[i].match(
                      /は(.*?)発動/
                    )
                }
                ;[dummy, resultData.Damage] = result.match(
                  /は(.*?)のダメージを与えた/
                )
  ...
  $.extend(CardFile.prototype, {
    ...
    save: function () {
      ...
      for (i = 0; i < this.cards.length; i++)
        datas.push(this.cards[i].getData())
      csaveData("meta-raid_cardfile_cards", datas, 1, 1)
    },
    load: function () {
      this.update_time = new Date(
        cloadData("meta-raid_cardfile_update_time", 0, 1)
      )
    ...
    init: function ($html, callback) {
      if (
        cloadData("meta-raid_cardfile_update_time", undefined, 1) ===
          undefined ||
...
*/ this.save()
    ...
    getReccomend: function (type) {
      ...
      for (var i = 0; i < this.cards.length; i++) {
        ...
        if (cloadData("meta-raid_unlimit_cost1", 0, 1) && card.cost == 1)
          continue
        if (card.level < cloadData("meta-raid_limit_lv_num", 0, 1)) continue
        if (card.ex_beat < 101) continue
        if (
          cloadData("meta-raid_ignore_flg", 0, 1) &&
          $.inArray(card.tid, ig_list) != -1
        )
          continue

        switch (type) {
          case "攻撃戦":
            card.pow = card.pow_atk
            break
          case "知力戦":
            card.pow = card.pow_int
            break
          case "防御戦":
            card.pow = card.pow_def
            break
          case "速度戦":
            card.pow = card.pow_spd
            break
        }
        ...
        card.pow =
          card.pow * ((card.ex_beat + rare_pow) / 100) * (1 + skill_pow / 100)

        if (cloadData("meta-raid_limit_power", 0, 1) && card.pow <= 67000)
          continue
        if (max_pow < card.pow) {
          max_pow = card.pow
          max_tid = card.tid
  ...
  var Raid = {
    ...
    pauseLink: function () {
      ...
      var $pause = $("<input>")
        .attr({
          type: "button",
          value: default_value[raid_pause],
          id: "raid_pause",
    ...
    logLink: function () {
      ...
      if (cloadData("meta-raid_raid_log", 0, 1) == 0) {
    ...
    configLink: function () {
        ...
        .on("click", function () {
          ...
          $("#IDlist").append(
            $(
              '<li><div id="IDlist_' +
                $("#IDlist li").length +
                '" class="input-group"><input class="form-control" value="0"></div></li>'
            )
          )
      ...
      var $idUL = $("<UL>").attr({ id: "IDlist", class: "list-unstyled" })
      var $ignoreList = $("<div>")
      var $ignoreInput = $("<input>").attr({
        type: "checkbox",
        name: "ignore_flg",
        value: "ignore_flg",
        id: "ignore_flg"
      })
      if (cloadData("meta-raid_ignore_flg", 0, 1))
        $ignoreInput.attr({ checked: "checked" })
      ...
      var $setting = $("<input>")
        .attr({
          ...
          value: "討伐戦設定",
          id: "imi_raid_setting",
          class: "left damage_ranking_updater",
          style: "width:100px;height:30px"
        })
        .on("click", function () {
          ...
          var html =
            ...
            "<div>オート有効" +
            '<div><input type="checkbox" name="auto_type" value="auto_attack" id="auto_attack"' +
            (auto_attack ? " checked " : "") +
            ">攻撃戦 " +
            '<input type="checkbox" name="auto_type" value="auto_deffence" id="auto_deffence"' +
            (auto_deffence ? " checked " : "") +
            ">防御戦 " +
            '<input type="checkbox" name="auto_type" value="auto_speed" id="auto_speed"' +
            (auto_speed ? " checked " : "") +
            ">速度戦 " +
            '<input type="checkbox" name="auto_type" value="auto_int" id="auto_int"' +
            (auto_int ? " checked " : "") +
            ">知力戦 </div></div>" +
            '<div><input type="checkbox" name="horihori_man"' +
            (cloadData("meta-raid_horihori_man", 0, 1) ? " checked " : "") +
            ">掘るので２枠のみで殴る</div>" +
            '<label>自分の戦闘判定用　自分の名前<input type="text"  name="horihori_owner" value="' +
            horihori_owner +
            '"></label>' +
            '<div><input type="checkbox" name="unlimit_cost1"' +
            (cloadData("meta-raid_unlimit_cost1", 0, 1) ? " checked " : "") +
            ">コスト1でも殴る</div>" +
            '<label>殴る最低レベル<input type="text"  name="limit_lv_num" value="' +
            limit_lv_num +
            '"></label>' +
            '<div><input type="checkbox" name="limit_power"' +
            (cloadData("meta-raid_limit_power", 0, 1) ? " checked " : "") +
            ">能力の低い武将では殴らない</div>" +
            '<div><input type="checkbox" name="target_world"' +
            (cloadData("meta-raid_target_world", 0, 1) ? " checked " : "") +
            ">ワールドを対象とする</div>" +
            '<div><label>叩きに行く  同盟ボーナス値<input type="text"  style="width:60px;" name="limit_bonus" value="' +
            limit_bonus +
            '"></label>' +
            '<label>  討伐武将のHP<input type="text" style="width:120px;" name="limit_hprem" value="' +
            limit_hprem +
            '"></label></div>' +
            '<div><label>叩きに行く体力<input type="text" style="width:60px;"  name="rem_stamina" value="' +
            rem_stamina +
            '"></label></div>' +
            '<div><label>デッキファイル更新間隔(秒)<input type="text" style="width:60px;"  name="periodic_time" value="' +
            periodic_time +
            '"></label></div>' +
            ""
          ...
          Display.dialog({
            ...
            buttons: {
              決定: function () {
                ...
                csaveData(
                  "meta-raid_auto_attack",
                  $('[id="auto_attack"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_auto_deffence",
                  $('[id="auto_deffence"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_auto_speed",
                  $('[id="auto_speed"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_auto_int",
                  $('[id="auto_int"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_horihori_man",
                  $('[name="horihori_man"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_horihori_owner",
                  $('[name="horihori_owner"]').val(),
                  1
                )
                csaveData(
                  "meta-raid_unlimit_cost1",
                  $('[name="unlimit_cost1"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_limit_lv_num",
                  $('[name="limit_lv_num"]').val(),
                  1
                )
                csaveData(
                  "meta-raid_limit_power",
                  $('[name="limit_power"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_target_world",
                  $('[name="target_world"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_ignore_flg",
                  $('[name="ignore_flg"]').prop("checked"),
                  1
                )
                csaveData(
                  "meta-raid_limit_bonus",
                  $('[name="limit_bonus"]').val(),
                  1
                )
                csaveData(
                  "meta-raid_limit_hprem",
                  $('[name="limit_hprem"]').val(),
                  1
                )
                csaveData(
                  "meta-raid_periodic_time",
                  $('[name="periodic_time"]').val(),
                  1
                )
                csaveData(
                  "meta-raid_rem_stamina",
                  $('[name="rem_stamina"]').val(),
                  1
                )
  ...
  Page.registerAction("card", "event_battle_top", {
    ...
    main: function () {
      ...
      // 討伐数とかを初期非表示に
      // $('#show_punitive').hide();
      $("#npcTitle").click(function () {
        $("#show_punitive3").toggle()
      })
      this.layouter($(".npcBusho"))

      var myself =
          $('#npcSearchBtn LI IMG[src$="btn_enter_battle.png"]').length > 0,
        alliance =
          $('#npcSearchBtn LI IMG[src$="btn_alliance_battle.png"]').length > 0,
        world =
          $('#npcSearchBtn LI IMG[src$="btn_world_battle.png"]').length > 0
      ...
      Util.wait(5000).done(function () {
        //console.log('test0001');

        //戦闘中の戦種を抽出
        var battle_cards = []
        $(".npcBusho").each(function () {
          battle_cards.push(new TargetBusho(this))
        })

        var battle_type01 = ""
        var battle_type02 = ""
        if (battle_cards.length > 0) {
          battle_type01 = battle_cards[0].type
        }
        if (battle_cards.length > 1) {
          battle_type02 = battle_cards[1].type
        }
        ...
        for (var i = 0; i < battle_cards.length; i++) {
          if (
            cloadData("meta-raid_horihori_owner", "", 1) ==
            battle_cards[i].discover_man
          ) {
        ...
        if (
          $(".npcBusho").size() < max_Battle &&
          cur_stamina >= cloadData("meta-raid_rem_stamina", 10, 1) - 0
        ) {
          var scope
          if (cloadData("meta-raid_target_world", 0, 1)) scope = 3
          else scope = 2
          //ここでターゲットを探してあったら遷移
          self.autoPager2(
            "/card/event_battle_top.php?p=1&filter_hp=2&scope=" + scope,
            battle_type01,
            battle_type02,
            owner_battle_Flg
          )
          ...
          if (cards.length > 0) {
            var targetBusyo = cards.filter(function (c) {
              ...
              switch (c.type) {
                case "攻撃戦":
                  auto_flg = cloadData("meta-raid_auto_attack", 1, 1)
                  break
                case "知力戦":
                  auto_flg = cloadData("meta-raid_auto_int", 1, 1)
                  break
                case "防御戦":
                  auto_flg = cloadData("meta-raid_auto_deffence", 1, 1)
                  break
                case "速度戦":
                  auto_flg = cloadData("meta-raid_auto_speed", 1, 1)
                  break
              }
              ...
              return (
                c.atk > 0 &&
                (c.bonus == 200 || cloadData("meta-raid_target_world", 0, 1)) &&
                auto_flg
              )
            })
            if (targetBusyo.length > 0) {
              ...
              var targetBusyo_02 = cards.filter(function (c) {
                ...
                console.log(
                  "raid_limit_bonus:" + cloadData("meta-raid_limit_bonus", 0, 1)
                )
                console.log(
                  "raid_limit_hprem:" +
                    cloadData("meta-raid_limit_hprem", 200000, 1)
                )
                console.log(
                  "raid_target_world:" +
                    cloadData("meta-raid_target_world", 0, 1)
                )
                ...
                return (
                  c.atk > 0 &&
                  (c.hprem - 0 <=
                    cloadData("meta-raid_limit_hprem", 200000, 1) - 0 ||
                    c.bonus - 0 >=
                      cloadData("meta-raid_limit_bonus", 0, 1) - 0) &&
                  auto_flg
                )
              })
      ...
      $.get(url, function (data) {
        ...
        .then(function () {
          ...
          if (
            ...
            autoPager2(
              "/card/event_battle_top.php?p=1&scope=4",
              battle_type01,
              battle_type02,
              owner_battle_Flg
            )
          ...
          if (cards.length > 0) {
            var targetBusyo = cards.filter(function (c) {
              ...
              return (
                c.target &&
                c.atk == 3 &&
                c.type !== battle_type01 &&
                c.type !== battle_type02 &&
                auto_flg
              )
            })
    ...
    layouter: function ($list) {
      $list.each(function () {
        ...
        $this.find('DT:contains("戦闘No")+DD').replaceWith(function () {
          var text = $(this).text()
          return (
            "<dd><a href=/card/event_battle_attack.php?entry_id=" +
            text +
            ">" +
            text +
            "</a></dd>"
  ...
  Page.registerAction("card", "event_battle_attack", {
    ...
    layouter: function () {
      var self = this
      var rem = $(".remaining_battle_count")
        .text()
        .match(/(\d)\/\d/)[1]
      var label = $("#l").val()
    ...
    returnTop: function (temp) {
      ...
      Util.wait(3 * 60 * 1000).done(function () {
        location.href = location.origin + "/card/event_battle_top.php"
      })
    },
    // 自動攻撃
    ...
    autoAttack: function (deck) {
      ...
      var bonus =
        $(".damage_bonus div:eq(2)")
          .text()
          .match(/([\d]+)%/)[1] - 0
      var remain =
        $(".remaining_battle_count")
          .text()
          .match(/(\d)\/\d/)[1] - 0
      var type = $("#infoBox")
        .text()
        .match(/ダメージ判定：(.*)/)[1] // 戦闘タイプ
      var nop = $("#infoBox span:eq(2)")
        .text()
        .match(/(\d+)\/(\d+)/)[1] // 参戦人数
      var $alliance = $(
        '#busho_desk_Box .battle_Column_btn_v2:first A[onclick^="return changeScope"]'
      ) // 同盟公開ボタン
      ...
      console.log(
        "raid_limit_bonus:" + cloadData("meta-raid_limit_bonus", 150, 1)
      )
      console.log(
        "raid_limit_hprem:" + cloadData("meta-raid_limit_hprem", 200000, 1)
      )
      console.log(
        "raid_target_world:" + cloadData("meta-raid_target_world", 0, 1)
      )
      ...
      // 残り攻撃回数がある場合は、200%か参戦人数20人以上の場合に殴る
        ...
        remain > 0 &&
        (bonus >= cloadData("meta-raid_limit_bonus", 150, 1) - 0 ||
          cloadData("meta-raid_target_world", 0, 1) ||
          hprem <= cloadData("meta-raid_limit_hprem", 200001, 1) - 0)
      ) {
        var card = deck.getReccomend(type)
        var eid = $("#entry_id").val()
