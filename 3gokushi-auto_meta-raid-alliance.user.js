// ==UserScript==
// @name           3gokushi-AUTO-meta-raid-alliance
// @description    ブラウザ三国志でレイドが捗るかもしれないツール
// @version        2.3.0.0a
// @namespace      3gokushi-AUTO-meta-a
...
//		if(type === "知力戦" && hpmax >= 6000000)this.target = 1;
		if(hpmax >= 4000000)this.target = 1;  //レアのHP変更により変える必要ありそう
	},
});
...
//console.log("name:"+card.name +" LV:"+card.level + " beat:"+ card.beat + " ex_beat:"+ card.ex_beat);
				...
				if(cloadData('meta-raid_limit_power',0,1) && card.pow <= 250000)continue;
				if(max_pow < card.pow){
					max_pow = card.pow;
					max_tid = card.tid;
					max_ref = i;
...
console.log("削除");
			...
			.on( 'click', function() {
				...
				var limit_lv_num=cloadData('meta-raid_limit_lv_num',200,1);
				var limit_bonus=cloadData('meta-raid_limit_bonus',0,1);
				var limit_hprem=cloadData('meta-raid_limit_hprem',400000000,1);
...
//				'<div><input type="checkbox" name="priority_300"'  + (storage.get('priority_300') ?' checked ' : '') + '>オススメ武将は討伐300を優先する</div>' +
				...
				'<div><input type="checkbox" name="target_world"'  + (cloadData('meta-raid_target_world',0,1) ?' checked ' : '') + '>同盟内を対象とする</div>' +
...
console.log("time up!");
				...
				if ($('.npcBusho').size()<max_Battle && cur_stamina >= cloadData('meta-raid_rem_stamina',10,1)-0) {
					...
					var scope;
					if(cloadData('meta-raid_target_world',0,1)) scope=2; else scope=4 ;
					//ここでターゲットを探してあったら遷移
...
console.log("raid_limit_hprem:"+cloadData("meta-raid_limit_hprem",400000000,1));
console.log("raid_target_world:"+cloadData("meta-raid_target_world",0,1));

								if(c.atk == 3 && cur_stamina < cloadData('meta-raid_rem_stamina',10,1)-0)auto_flg = 0;
							 return (c.atk>0 && (c.hprem-0 < cloadData('meta-raid_limit_hprem',400000000,1)-0 || c.bonus-0 >= cloadData('meta-raid_limit_bonus',0,1)-0) && auto_flg) ; 
							});
...
console.log("pager2:"+url);
			...
			.then( function(  ) {
				...
				if(cloadData('meta-raid_target_world',0,1) && !$html.find('.npcBusho')){
					autoPager2('/card/event_battle_top.php?p=1&scope=2', battle_type01 , battle_type02 ,owner_battle_Flg);
					return;
				}
				// 武将情報の取得
...
console.log("raid_target_world:"+cloadData("meta-raid_target_world",0,1));
			...
			// 残り攻撃回数がある場合は、HP2億以下で参戦人数0人以上のものを殴る
			else if((remain>0) && ((bonus>=cloadData('meta-raid_limit_bonus',0,1)-0)||(cloadData("meta-raid_target_world",0,1))||hprem>=cloadData('meta-raid_limit_hprem',400000000,1)-0)) {
				var card = deck.getReccomend(type);
				var 	eid = $('#entry_id').val();
